...
void SetIdleState(void)
{
  ...
  for (Bay = LeftBay; Bay <= RightBay; Bay++) {
    ...
    UART_TxNewLine();
    SetVolume(Settings.ProdDef[BayProduct[Bay]].IdleVolume);
  }
  Bay = TempBay;
...
void ProcessSelectKey(Uint8 Key)
{
  ...
  if (!BayNotSource) {   // If selection source selection
    ...
    if (IdleTime &&                                             // If not idle
      ...
      if ((HeadphoneBay == NoBay) && (IdleTime != 1)) {
...
int main(void)
{
  ...
  wdt_reset();//feed the watchdog
  SetBit(SD_PWR_PORT,SD_PWR);
  DelayMS(100);
  SetLamp(4);
  Timer_Clear();
  if(!MP3_Init())
  {
    UART_TxStr("\r\nMP3 init fail need wait for watchdog restart\r\n");
    DelayMS(10000); //watch dog
  }
  MP3_Volume(250);
...
void MainLoop(Uint8 PlayerStatus)
{
  ...
  if (PlayerStatus == 0) {
    ...
    if(!MP3_Track(Track)){
      ...
      //not success open track need restart
      DelayMS(10000); //watch dog
    }
  }
