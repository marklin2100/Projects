...
{
	int r;
  UART_TxStr("MP3_Init, Initialize SPI\r\n");
  SPI_Init();
  ...
  UART_TxStr("Initialize SD card module\r\n");
  if (sd_init() != 0) {
...
// Set volume
  ...
  UART_TxStr("Set audio volume is mute\r\n");
  vs_sci_write(0x0b, 0xfefe);//mute  = 0xFEFE  0x0000 = max volume
