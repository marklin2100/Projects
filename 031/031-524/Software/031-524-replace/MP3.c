...
bool MP3_Init(void)
{
  ...
  UART_TxStr("MP3_Init, initialising SPI\r\n");
  wdt_reset();
  SPI_Init();
  ...
  UART_TxStr("Initialising SD card module\r\n");
  if (sd_init() != 0) {
    UART_TxStr("Failed to initialise SD card\r\n");
  }
...
//  vs_sci_write(0x03, 0x6800);
  ...
  r = diet_init();
  if (r != 0) {
    UART_TxStr("Failed to initialise FAT driver: ");
    UART_TxUint16(r);
    UART_TxNewLine();
    return false;
  }
